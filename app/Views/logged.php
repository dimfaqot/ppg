...
<body style="margin-bottom: 70px;" class="bg-light">
    ...
    <script>
        ...
        $(document).on('keyup', '.cari_laporan', function(e) {
            e.preventDefault();

            let value = $(this).val().toLowerCase();

            $('.search_laporan').filter(function() {
        ...
        $(document).on('click', '.confirm', function(e) {
            ...
            confirm(controller, tabel, message, method, id, job_id);
        });
        ...
        $(document).on('click', '.delete', function(e) {
            ...
            let id = $(this).data('id');
            let job_id = $(this).data('job_id');
            hapus(controller, method, tabel, id, job_id);
        });
        $(document).on('change', '.update_check', function(e) {
            e.preventDefault();
            let controller = $(this).data('controller');
            ...
            let tabel = $(this).data('tabel');
            let id = $(this).data('id');
            update_check(controller, method, tabel, col, id);
        });

        $(document).on('keyup', '.rupiah', function(e) {
            e.preventDefault();
            let val = $(this).val();
            $(this).val(rupiah(val.toString()));
        });

        $(document).on('change', '.add_tugas', function(e) {
            e.preventDefault();
            let val = $(this).val();
            let id = $(this).data('id');

            $('.add_fee_' + id).val(rupiah(val));
        ...
        $(document).on('change', '.update', function(e) {
            ...
            if (col == 'tugas') {
                fee = val;
                val = $('.update_tugas_' + id + ' option:selected').text();

            }
            update_crew(id, job_id, val, col, fee);
        });
        $(document).on('change', '.update_ket', function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            let val = $(this).val();
        ...
        $(document).on('blur', '.update_fee', function(e) {
            e.preventDefault();
            let id = $(this).data('id');
            let job_id = $(this).data('job_id');
            let val = $(this).val();

            update_fee(id, job_id, val);
        });
        ...
        $(document).on('click', '.body_accord', function(e) {
            e.preventDefault();
            let i = $(this).data('i');


            if ($(this).hasClass('accord_active')) {
                $(this).removeClass('accord_active');
                if (i % 2 == 0) {
                    $(this).addClass('accord_light');
                } else {
                    $(this).addClass('accord_dark');
                }
            } else {
